...
def test_printRes(data,w1,w2,w3,w4):
    data1x=[]
    data1y =[]
    data2x =[]
    data2y =[]
    ...
    for i in range(len(data[1, :])):
        ...
        if(i<2000):
            ...
            if (abs(z - 0) > 0.5):
                data[2, i] = 1
                data1xT.append(data[0, i])
                data1yT.append(data[1, i])
                a1T = a1T + 1
            else:
                data[2, i] = 0
                data2xT.append(data[0, i])
                data2yT.append(data[1, i])
                a2T = a2T + 1


    plt.plot(data1x, data1y, 'b.')
    plt.plot(data2x, data2y, 'r.')
    plt.plot(data1xT, data1yT, 'g.')
    plt.plot(data2xT, data2yT, 'y.')
    plt.show()
def main():
    matfn = 'Data-Ass2.mat'
    ...
    while(error<>0):
        ...
        if(c%100==0):
            print c
        # print c
        for i in range(len(data[1,0:2000])):
            y1=activation(w1,data[0,i],data[1,i])
            y2 = activation(w2, data[0, i], data[1, i])
            y3 = activation(w3, data[0, i], data[1, i])
            z = activation2(w4, y1, y2, y3)
    ...
    test_printRes(data,w1,w2,w3,w4)
